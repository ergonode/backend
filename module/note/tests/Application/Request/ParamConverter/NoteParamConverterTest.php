...
namespace Ergonode\Note\Tests\Application\Request\ParamConverter;

use Ergonode\Note\Domain\Entity\Note;
use Ergonode\Note\Domain\Repository\NoteRepositoryInterface;
use Ergonode\Note\Application\Request\ParamConverter\NoteParamConverter;
...
class NoteParamConverterTest extends TestCase
{
    /**
    ...
    public function testSupportedClass(): void
    {
        ...
        $this->configuration->method('getClass')->willReturn(Note::class);

        $paramConverter = new NoteParamConverter($this->repository);
    ...
    {
        $this->request->method('get')->willReturn(Uuid::uuid4()->toString());
        $this->repository->method('load')->willReturn($this->createMock(Note::class));
