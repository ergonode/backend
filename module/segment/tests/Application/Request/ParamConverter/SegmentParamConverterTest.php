...
namespace Ergonode\Segment\Tests\Application\Request\ParamConverter;

use Ergonode\Segment\Application\Request\ParamConverter\SegmentParamConverter;
...
/**
 */
class SegmentParamConverterTest extends TestCase
{
    /**
    ...
    public function testSupportedClass(): void
    {
        ...
        $this->configuration->method('getClass')->willReturn(Segment::class);

        $paramConverter = new SegmentParamConverter($this->repository);
    ...
    {
        $this->request->method('get')->willReturn(Uuid::uuid4()->toString());
        $this->repository->method('load')->willReturn($this->createMock(Segment::class));
